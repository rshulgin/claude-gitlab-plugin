...
export const projectTools = (client: GitLabClient) => [
  {
    name: "gitlab_list_projects",
    description:
      "List GitLab projects accessible by the authenticated user. Can filter by owned, membership, or search.",
...

  {
    ...
    description:
      "Get detailed information about a specific GitLab project, including statistics, default branch, and URLs.",
    inputSchema: z.object({
      project_id: z
        .union([z.string(), z.number()])
        .describe(
          "Project ID (number) or URL-encoded path (e.g. 'mygroup/myrepo')"
        ),
    }),
    handler: async (input: { project_id: string | number }) => {
