...
export const pipelineTools = (client: GitLabClient) => [
  {
...

  {
    ...
    description:
      "Get the console output (log) of a CI/CD job. Useful for debugging failed jobs.",
    inputSchema: z.object({
      project_id: z.union([z.string(), z.number()]).describe("Project ID or path"),
      job_id: z.number().describe("Job ID"),
    }),
    handler: async (input: { project_id: string | number; job_id: number }) => {
